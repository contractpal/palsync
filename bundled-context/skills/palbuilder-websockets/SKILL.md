...
Chat is just the round-trip with one twist: the receiver, instead of echoing to the **one**
socket that sent the message, **broadcasts it to every socket on the same channel**. The
channel *is* the room. This composes the verified primitives above — `createClientSocket`
(§1), per-message receiver (§3), `getChannel()`, and the `socketGuid` broadcast (§5) — nothing
new.
