...
| `roles` | Role names registered on the pal for document signatures. **Shape is `{ "string": ["signer", "approver"] }` — the same wrapper convention as `DataList.cols`/`DatasetIndex.columns`/`Data.values.entry` — not a bare array.** `""` when empty. A bare JS array (`roles: ["signer"]`) LOOKS like it pushes fine (`pal_push` reports `ok:true`) but silently fails to save: it serializes to repeated top-level `<roles>` XML elements instead of one `<roles>` wrapper containing `<string>` children, so the server's deserializer sees nothing, the "no roles associated" warning still fires in that same push response, and the next pull shows `roles` reset back to `""`. Use the wrapped shape and it round-trips correctly. |
