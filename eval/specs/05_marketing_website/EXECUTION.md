...
1. Hand-build the web shell as a standalone first step: create the page shell, copy the four canonical design-system files verbatim, author and register the new pal's readable `styles.css`, and establish the initial web workflow from the palbuilder workflow guidance.
...
|---|---|---|---|---|---|---|
| T1 | hand-build web shell, design-system files, styles.css, and initial workflow | cheap | §3, §6, §10 | — | todo | Web page shell, initial workflow, four canonical files copied verbatim, and readable `styles.css` are present and registered in pal.json; pal_validate 0; pal_test web workflow VALIDATED on the hand-built shell |
