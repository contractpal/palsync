...
function syncDetails(palName, { cli = false, skillsDir = ".claude/skills" } = {}) {
    ...
    return [
        ...
        "workflow API, field/signature shapes, and the `c:document` tag). An HTML document places",
        "fields with named inputs directly in its markup — fully self-service. A PDF document's field/",
        "signature `x`/`y`/`width`/`height`/`page` coordinates come from the PalBuilder designer —",
        "never hand-guess them; say so if you need one placed and don't have designer access:",
        "```json",
