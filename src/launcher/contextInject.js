...
// Per-skill companion assets (files beside SKILL.md that must ship into the workspace).
// Most skills are SKILL.md-only; only names listed here carry extras. copySkillSet copies
// SKILL.md + each asset, so a SKILL that references references/* MUST be listed here or the
// referenced file never reaches the agent.
const SKILL_ASSETS = {
    "pal-spec": ["references/palbuilder-types.md"],
    "palbuilder-frontend": ["references/tag-reference.md", "references/js-and-patterns.md"],
    "palbuilder-backend": ["references/api-reference.md"],
    "palbuilder-websockets": ["references/worked-example-chat.md"],
    "palbuilder-jobs-http": ["references/advanced-patterns.md"]
};
...
function effectiveSkills(withSeo = false) {
    return ALWAYS_ON_SKILLS.map(name => ({ name, assets: SKILL_ASSETS[name] || [] }))
...
function ownedSkillNames() {
    return new Set([...ALWAYS_ON_SKILLS, ...SEO_SKILLS.map(s => s.name), ...RETIRED_SKILLS]);
}
