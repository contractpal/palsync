...
function syncDetails(palName, { cli = false, skillsDir = ".claude/skills" } = {}) {
    ...
    return [
        ...
        "design pass: follow the active verification policy for render proof. Standard uses one",
        "affected viewport when needed; Thorough adds the desktop/mobile pair. Inspect `designAudit`",
        "and re-render only after fixing a visible failure.",
        ...
        "4. When workflow code changed and the verification plan calls for compile proof, run **" + T.test + "** —",
        "   it uses the SERVER's own validation. It does NOT open the user's browser by default; open a",
        "   live preview only when the user asks (" + (cli ? "`palsync test --preview` or " + T.open : "`pal_test` with `preview:true`") + ").",
        "5. **" + T.validate + " and " + T.test + " only prove the code COMPILES — not that it RENDERS.** A",
        "   workflow that validates can still THROW at runtime (bad SQL, a null, a missing column) and paint",
        "   an error block instead of the UI, so a visible change needs render evidence: " + T.screenshot + "",
        "   (it drives an authenticated console screen too and returns a PNG + a `renderError` if the workflow",
        "   threw), or " + T.fetch + "/" + T.preview + " with `expect:[strings]` for server-rendered WEB text.",
        "   How much of this a given change needs is decided by the verification policy (see the managed",
        "   contract above), not by running everything every time. These tools show the LAST PUSHED version.",
        ...
        "   - Only ask the user to eyeball a screen when the check tool reports itself unavailable (no",
        "     Chromium, or the auth replay failed) — never as a substitute for calling it.",
        "6. When the verification plan calls for SEO proof on a PUBLIC WEB page, run **" + T.seo + "** after",
        "   pushing — it checks title/description, ABSOLUTE og: URLs, one H1, JSON-LD, and img alt text.",
        "   Do not call a reported error a pass. (Read the palbuilder-seo skill first when it\'s present.)",
        "",
        ...
        "`palsync completion check` reports whether the work is complete and which final-review path the",
        "user's `review` setting asks for.",
        "",
        ...
        "  workflow is NOT attached automatically — call `" + "pal_debug" + "` yourself whenever you actually want it:",
        "  right after a run you're diagnosing, after a delayed/system workflow finishes, or after watching",
        "  a human navigate the site. When diagnosing a runtime fault, add `c.debug` lines to the workflow,",
        "  push, re-run the check, call `" + "pal_debug" + "`, read the output — then REMOVE the instrumentation",
        "  before declaring the work done. The buffer is consume-once PER YOUR OWN Chip session (tagged by",
        "  Chip-Session-ID): reading it clears only your copy, not the human's PalBuilder view or another",
        "  Chip session's, so you don't need to coordinate with anyone else watching the same pal.",
        ]),
        ...
        "  actually work\"). When the verification plan calls for behavior proof after create/edit/delete, use it;",
        "  after an edit put the OLD value in `absent` to catch a duplicate insert; after a delete put the deleted name in `absent`, never",
        ...
        "",
        "**When the human mentions \"clipboard\" or a \"screenshot\" they want you to look at, check",
        "`./.clipboard/` in the project directory first.** Chip's GUI has a clipboard bridge (a titlebar",
        "button) that saves whatever the human copies into `.clipboard/<random-name>.png` or `.txt` —",
        "look there before asking them to paste content inline or describe an image in words. When",
        "asked to put something \"on the clipboard\" for the human to see, or to \"copy\" something",
        "there, **write a file into `.clipboard/` — a normal file write, nothing else.** \"The",
        "clipboard\" in this context means that folder, NOT the operating system's real clipboard:",
        "never run `clip`/`Set-Clipboard`/`pbcopy`/`xclip`/`xsel` or anything similar to change what's",
        "actually on the human's system clipboard — that overwrites content they may still need and is",
        "never what's being asked for here. Then tell the human in the console to check it — the same",
        "button has a matching \"Agent's Clipboard\" view showing whatever landed there most recently.",
        "",
...
function syncSection(palName, { cli = false, policy = policyDefaults, skillsDir = ".claude/skills" } = {}) {
    return [
        ...
        "",
        "**If the human mentions \"clipboard\" or a \"screenshot,\" or asks you to put/copy something",
        "\"on the clipboard\" for them, this project has a `.clipboard/` folder for exactly that** —",
        "call `pal_context section:\"creating-files\"` for the exact rules before acting (short version:",
        "read/write a plain file there, never touch the human's real OS clipboard).",
        "",
