...
function syncSection(palName, { cli = false } = {}) {
    ...
    return [
        ...
        "   - **" + T.validate + " and " + T.test + " only prove the code COMPILES — not that it RENDERS.** A",
        "     workflow that validates can still THROW at runtime (bad SQL, a null, a column the table lacks)",
        "     and paint an error block instead of the UI. " + T.screenshot + " reads the rendered page and",
        "     reports a `renderError` when that happens (and " + T.fetch + "/" + T.preview + " `expect:[strings]`",
        "     catch it for web). A captured render with a `renderError` is a FAIL — fix it before calling the",
        "     screen done. Do not declare a build complete on validate+test alone.",
