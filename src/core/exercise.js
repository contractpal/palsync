...
async function runExercise(session, guid, { steps, workflow, viewport } = {}) {
    ...
    if (problems.length) return { ran: false, invalid: true, problems };
    ...
    if (!isWeb && !needsBrowser(steps) && steps.some(s => s.action || s.page)) {
        return { ran: false, kind: t.kind, invalid: true,
    ...
    if (isWeb && !needsBrowser(steps)) return exerciseByFetch(t, steps);
    return exerciseByBrowser(t, steps, viewport);
}
    ...
    try {
        ...
        for (let i = 0; i < steps.length; i++) {
            ...
            try {
                ...
                if (step.fill) {
                    for (const [name, value] of Object.entries(step.fill)) {
                        ...
                        if (await pg.locator(sel).count() === 0) return fail("no input named \"" + name + "\" on the current screen — check the fragment's name= attributes");
                ...
                if (step.click) {
                    ...
                    if (await loc.count() === 0) return fail("nothing to click matching \"" + c + "\" on the current screen (match the element's exact visible text, or use #id/.class)");
                    await loc.first().click();
                    await pg.waitForLoadState("networkidle");
                    await pg.waitForTimeout(400); // let an ajax-target swap settle
                }
            } catch (e) {
            ...
            results.push({ step: i + 1, label: stepLabel(step), pass,
                           expect: chk.expect, absent: chk.absent, renderError, url: sanitizeUrl(pg.url()) });
...
module.exports = { runExercise, validateSteps, checkStep, stepLabel, needsBrowser, formatExercise, MAX_STEPS };
