...
// A QUERY_DATASET response's DatasetQueryResult.data is the SAME heterogeneous-sibling-tag
// pattern as a DataList row — each <string-array> row is a sequence of <string>/<null/> cells —
// so the primary parse groups a row's non-null cells away from its null cells and loses which
// COLUMN each value belongs to. Server-verified (Audithelm V1 / activityLog, 2026-09-02): a row
// with a null actorProfileId came back as { string: [id, workspaceId, action, ...], null: "" },
// which read naively would shift `action` into the actorProfileId column.
function readOrderedQueryResultRows(ordered) {
    const queryResult = findOrderedDeep(ordered, "queryResult");
    if (!queryResult) return null;
    const dataChildren = orderedChildrenByTag(queryResult, "data")[0];
    if (!dataChildren) return null;
    return orderedChildrenByTag(dataChildren, "string-array").map(cellNodes => cellNodes.map(cellText));
}

// Patch every DataList entry's recs, every Data entry's values, and any DatasetQueryResult's data
// rows in `parsedResult` (the object CloudPistonXMLParser().parse() produced) in place, using data
// reconstructed from the same raw XML. No-op (and skips the extra parse entirely) when the
// response carries none of those sections.
function fixOrderSensitiveShapes(xmlString, parsedResult) {
    ...
    const queryResultSection = findKeyDeep(parsedResult, "queryResult");
    const hasQueryRows = !!queryResultSection && !!queryResultSection.data && queryResultSection.data["string-array"] !== undefined;
    if (!hasDataLists && !hasData && !hasQueryRows) return parsedResult;

    const ordered = readOrderedTree(xmlString);

    if (hasQueryRows) {
        const rows = readOrderedQueryResultRows(ordered);
        if (rows) queryResultSection.data = { "string-array": rows.map(cells => ({ string: cells })) };
    }
