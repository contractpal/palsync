...
class CloudPistonAPIManager {
    static async fetchAPI(session, endpoint, headers, task) {
        ...
        // Identifies which specific agent+pal window this request came from — set by Chip
        // (never by the plain CLI) so the server can eventually distinguish concurrent Chip
        // sessions on the same pal instead of treating every request as the same anonymous
        // client. Absent entirely when session.chipSessionId isn't set.
        if (session.chipSessionId) headers.append("Chip-Session-ID", session.chipSessionId);
    ...
    // `serverData` is the base64 of the accumulated debug text. CONSUME-ONCE PER TIER: fetchAPI
    // (above) tags the request with Chip-Session-ID when session.chipSessionId is set, so the
    // server keeps a second-tier debug buffer per agent session — reading clears only that
    // session's own copy, not the human's PalBuilder IDE view or any other Chip session's.
