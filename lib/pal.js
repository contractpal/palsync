...
const CONTENT_TYPES = [
    ...
    { folder: "wizards", key: "Wizard" },
];

class Pal {
    constructor(init) {
        // Preserve every top-level field verbatim first (storeSettings, consoleSettings,
        // fonts, automatedScripts, mobileConfigurations, desktopBindings, trashCan,
        // readme, releaseNotes, secureFields, and any future server field this class doesn't
        // know about) so pull -> push round-trips them unchanged. The known fields below are
        // then normalized/overridden on top.
        ...
        this.wizards = init.wizards !== '' ? init.wizards : { entry: [] };
    ...
    static toXml(pal) {
        return buildOrderSensitiveShapes(pal, true);
    }
    //#endregion STATIC
    ...
    get allWizards() { return this.wizards?.entry ?? []; }
    ...
    async injectFileContent() {
        ...
        await Promise.all([
            ...
            inject(this.allWizards, "Wizard", "wizards"),
        ]);
