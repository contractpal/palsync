...
`palsync setup --agent pi` installs the native extension at `~/.pi/agent/extensions/palsync/`. Pi loads its TypeScript entry directly; no build step is required. After updating PalSync, rerun setup and reload Pi to install updated tool metadata.

The extension exposes `pal_tools`, `pal_context`, `pal_validate`, `pal_spec_lint`, and `pal_stats` initially. All PalSync tool schemas are registered up front, as Pi's deferred-loading contract requires, but inactive schemas stay out of the prompt. `pal_tools({query})` activates matching tools additively using the deterministic groups `sync`, `browser`, `runtime`, `project`, and `spec`. Before calling a required inactive `pal_*` tool, query `pal_tools` once with its exact name; combine immediately needed names, but skip already-active tools. Its result immediately confirms that activated tools are callable on the next model response and states any `pal_impact`/`pal_ast` routing guidance. Pi keeps the same guidance in the system Guidelines while those tools are active. The agent must attempt the required activated tool directly; activation alone is not evidence of failure and does not justify falling back to the PalSync CLI. The MCP server starts only when a PalSync tool is first called.
...
Model-level session usage needs no agent-visible bookkeeping call. On the first agent turn the extension records either a build phase or, for the exact expanded bundled `pal-review` skill, a review phase; it closes build at `palsync session-summary`, review at `palsync review check`, or the active phase during awaited fail-open shutdown. It hands `pal_stats` current `ctx.sessionManager.getEntries()` counters and an internal session identity through MCP request `_meta` — transport metadata, not a tool parameter the model could invent values for. Historical windows and evidence remain preserved but are never reported as the current session.
